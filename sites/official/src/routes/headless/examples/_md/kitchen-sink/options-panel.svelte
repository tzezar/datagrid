<script lang="ts">
	...
	const handleColumnPinningChange = (column: Column<any>, position: PinningPosition) => {
		grid.columnManager.changeColumnPinningPosition(column, position);
		grid.columnManager.refreshColumnPinningOffsets();
	};

	const handleColumnResize = (column: Column<any>, width: number) => {
				...
				<div class="flex max-w-[300px] flex-row justify-between gap-2">
					...
					<div class="flex gap-4">
						...
						<button
							...
							disabled={!grid.columnManager.canMoveColumnRight(column) || column.movable === false}
						>
							<Collapse />
						</button>
					</div>

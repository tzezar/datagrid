...
<div class="grid-wrapper">
	<div class="grid-content">
		<VirtualList items={grid.rows} class="list-table" style="height:600px" isTable={true}>
			...
			{/snippet}
			{#snippet vl_slot({ item, index }: { item: Row<any>; index: number })}
				{#if item.groupId}
					<div class="grid-row">
						{#each grid.columns as column, colIndex}
							...
							>
								{#if column.columnId === 'actions'}
									<ActionsGroup {grid} row={item}/>
								{/if}

								{#if column.columnId === item.columnId}
									<!-- <span>{column.accessor(row)}</span> -->
									<span class="overflow-hidden text-ellipsis font-bold">{item.groupId}</span>
								{:else if item.aggregates[column.columnId]}
									{#each Object.entries(item.aggregates[column.columnId]) as [aggKey, aggValue]}
					...
					<div class="grid-row">
						{#each grid.columnManager.getVisibleColumns() as column}
							<div
								...
								style:flex-grow={column.size.grow ? 1 : null}
							>
								<CellRenderer {column} row={item} {grid} />
							</div>
						{/each}
					</div>

...
{#snippet GroupCellHeaderSnippet(column: LeafColumn<any>, row: GridGroupRow<any>)}
	...
	>
		<span class="rounded-sm border-[1px] border-primary/30">
		...
		<span class="flex place-items-center pl-1 text-xs leading-none text-muted-foreground">
			[{row.children.length}]
		</span>
	</button>
{/snippet}
...
{#snippet GroupCellAggregationSnippet(row: GridGroupRow<any>, column: LeafColumn<any>)}
	<div class="flex h-full items-center px-4 py-2 text-xs text-muted-foreground">

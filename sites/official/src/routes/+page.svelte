<script lang="ts">
  ...
  <div class="p-4">
	<div class="mb-4 flex flex-col gap-4 justify-center">
	  <div class="flex flex-col ">
		<label for="rowCount" class="text-sm mb-1 font-medium">Number of rows:</label>
		<input
		  id="rowCount"
		  type="number"
		  bind:value={rowCount}
	  ...
	  <button
		...
		disabled={isLoading}
		class="px-4 py-2 bg-orange-400 text-white rounded hover:bg-orange-500 disabled:opacity-50"
	  >
	...
	{#if isLoading}
	  <div>
		...
		<p class='text-xs pt-2'>If your device can't handle this, please let me know on github. I will adjust the number of rows.</p>
	  </div>
	{:else}
	  <Datagrid {data} />
	{/if}

...
export class SortingService extends BaseService {
		...
		const applyMultiColumnSort = () => {
			if (!isColumnSorted) {
				const isOverMaxColCount =
					datagrid.features.sorting.sortConfigs.length >=
					datagrid.features.sorting.maxMultiSortColumns;
				if (isOverMaxColCount) {
					...
					if (datagrid.features.sorting.sortConfigs.length > 0) {
						datagrid.features.sorting.removeSortConfig(
							datagrid.features.sorting.sortConfigs[0]!.columnId
						);
					}
				}
	...
	applyAscendingSort(column: LeafColumn<any>) {
		...
		if (isColumnSorted)
			this.datagrid.features.sorting.changeSortConfigDirection(column.columnId, 'ascending');
		else this.datagrid.features.sorting.addSortConfig(column.columnId, 'ascending');

		this.datagrid.events.emit('onColumnSort', { column });

		if (this.datagrid.features.sorting.isManual) {
			return;
		}
	...
	applyDescendingSort(column: LeafColumn<any>) {
		...
		const isColumnSorted = this.datagrid.features.sorting.isColumnSorted(column.columnId);
		if (isColumnSorted)
			this.datagrid.features.sorting.changeSortConfigDirection(column.columnId, 'descending');

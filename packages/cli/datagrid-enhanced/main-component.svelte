<script lang="ts">
    ...
    const leafColumnsToDisplay = $derived(
        leafColumns.filter((col) => !col.columnId.startsWith("_")),
    );
    ...
    const shouldAnimateHeaders = $derived(
        datagrid.extra.features.animations.shouldAnimateHeaders(),
    );
</script>
...
<Portal disabled={!isFullscreenEnabled}>
    ...
    >
        ...
        {@render ContainerSnippet(
            datagrid.extra.features.virtualization.enabled,
        )}
...
{#snippet BodyRowSnippet(row: GridRow<any>, rowIndex: number)}
    ...
    {:else}
        <div
            role="row"
            class={cn(
                datagrid.customization.styling.getBodyRowClasses(
                    row,
                    Number(rowIndex),
                ),
                "",
            ...
            use:identifier={{ datagrid, value: "row-" + row.identifier }}
            onclick={(e) =>
                datagrid.events.emit("onRowClick", { row, event: e })}
            ondblclick={(e) =>
                datagrid.events.emit("onRowDoubleClick", { row, event: e })}
            onkeydown={(e) => {
                if (e.key === "Enter")
                    datagrid.events.emit("onRowClick", { row, event: e });
            }}
            ...
            {#if datagrid.extra.features.animations.shouldAnimateRows()}
                {#each leafColumnsToDisplay as column (column.columnId)}
                    <div
                        ...
                        animate:flip={{
                            duration: (len) =>
                                datagrid.extra.features.animations.getRowsFlipDuration(
                                    len,
                                ),
                        }}
                    >
        ...
        {#if row.isExpanded()}
            ...
            {:else}
                <div
                    class={datagrid.customization.styling.getBodyRowExpandedClasses()}
                >
                    <div class="cell sticky left-0">
                        {#if expandedRowContent}
                            {@render expandedRowContent(row)}
                        {:else}
                            Place your content in the `expandedRowContent`
                            snippet
                        {/if}
                    </div>
                </div>
                ...
                {#if shouldAnimateHeaders}
                    {#each headerColumnsWithoutAdditional as column (column.columnId)}
                        <div
                            ...
                            animate:flip={{
                                duration: (len) =>
                                    datagrid.extra.features.animations.getHeadersFlipDuration(
                                        len,
                                    ),
                            }}
                        >
            ...
            {#each datagrid.rows.getVisibleRows() as row, rowIndex (row.identifier)}
                ...
                {:else}
                    <div
                        ...
                        onclick={(e) =>
                            datagrid.events.emit("onRowClick", {
                                row,
                                event: e,
                            })}
                        ondblclick={(e) =>
                            datagrid.events.emit("onRowDoubleClick", {
                                row,
                                event: e,
                            })}
                        onkeydown={(e) => {
...
{#snippet FooterSnippet()}
    ...
    {:else if footerContent}
        <div
            class={cn(
                datagrid.customization.styling.getFooterContainerClasses(),
            )}
        >
...
{#snippet AdditionalHeaderCells(position: "left" | "right")}
    ...
    {#if datagrid.extra.features.rowExpanding.position === position}
        <RowExpandingColumnHeaderCell
            {datagrid}
            column={headerColumns.find(
                (col) => col.columnId === "_expand",
            ) as LeafColumn<any>}
        />
    {/if}
{/snippet}

{#snippet AdditionalBodyCells(
    position: "left" | "right",
